...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch('/api/resume').then(r => r.json()).then(d => {
      ...
      if (d.success) setResumeUrl(d.url || '')
    })
  }, [])
  ...
  const deleteResume = async () => {
    ...
    const d = await fetch('/api/resume', { method: 'DELETE' }).then(r => r.json())
    if (d.success) { setResumeExists(false); setResumeUrl(''); flashUpload(true, 'Resume removed.') }
  }
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    setPwSaving(true)
    const d = await fetch('/api/password', {
      method: 'PUT',
      ...
      <div className="a-card" style={{ marginBottom: '1.5rem', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem', flexWrap: 'wrap' }}>
            ...
            {resumeExists && (
              <>
                <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="a-btn a-btn-outline-gold">
                  ↗ Preview Resume
                </a>
