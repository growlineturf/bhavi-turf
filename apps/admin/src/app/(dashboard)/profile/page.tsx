...
type Profile = {
  ...
  linkedin:string; github:string; openToWork:boolean; availabilityText:string
}
...
export default function ProfileAdmin() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const d = await fetch('/api/portfolio/profile', {
      ...
      <form onSubmit={save}>
        ...
        <div className="a-card" style={{ marginBottom:'1.25rem' }}>
          ...
          <div style={{ padding:'1.5rem', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.25rem' }}>
            {renderField({ label: 'Full Name', field: 'name', half: true, ph: 'Your full name' })}
            {renderField({ label: 'Job Title', field: 'title', half: true, ph: 'e.g. AI & Full-Stack Developer' })}
            {renderField({ label: 'Tagline', field: 'tagline', ph: 'Your short headline' })}
            {renderField({ label: 'Avatar Image URL', field: 'avatarUrl', ph: 'https://...' })}
            {renderField({ label: 'Professional Summary', field: 'summary', ta: true, ph: 'Write 2-3 sentences about yourself...' })}
          </div>
        </div>
        ...
        <div className="a-card" style={{ marginBottom:'1.25rem' }}>
          ...
          <div style={{ padding:'1.5rem', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.25rem' }}>
            {renderField({ label: 'Email', field: 'email', type: 'email', half: true, ph: 'your@email.com' })}
            {renderField({ label: 'Phone', field: 'phone', half: true, ph: '+91 XXXXX XXXXX' })}
            {renderField({ label: 'Location', field: 'location', half: true, ph: 'City, Country' })}
            {renderField({ label: 'LinkedIn URL', field: 'linkedin', half: true, ph: 'https://linkedin.com/in/username' })}
            {renderField({ label: 'GitHub URL', field: 'github', ph: 'https://github.com/username' })}
          </div>
        </div>
        ...
        <div className="a-card" style={{ marginBottom:'1.5rem' }}>
          ...
          <div style={{ padding:'0 1.5rem 1.5rem' }}>
            <label className="a-label">Availability Text</label>
            <input
              value={form.availabilityText}
              onChange={e => set('availabilityText', e.target.value)}
              placeholder="Open to full-time opportunities from 2026"
              className="a-input"
            />
          </div>
        </div>
